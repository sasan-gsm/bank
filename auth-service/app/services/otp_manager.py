...
class OTPManager:
    ...
    async def check_rate_limit(self, user_id: int, purpose: str) -> bool:
        ...
        await cache_manager.set_cache(
            cache_key, int(current_count) + 1, ttl=self.rate_limit_window
        )
        return True
    ...
    async def check_verification_attempts(self, user_id: int, purpose: str) -> bool:
        ...
        if int(current_attempts) >= self.max_attempts:
            logger.warning(
                f"Max verification attempts exceeded for user {user_id}, purpose {purpose}"
            )
            return False
    ...
    async def increment_verification_attempts(self, user_id: int, purpose: str) -> None:
        ...
        if current_attempts is None:
            await cache_manager.set_cache(
                cache_key, 1, ttl=self.otp_expire_minutes * 60
            )
        else:
            await cache_manager.set_cache(
                cache_key, int(current_attempts) + 1, ttl=self.otp_expire_minutes * 60
            )
    ...
    async def verify_otp(
        self, user_id: int, otp_code: str, purpose: str, db: AsyncSession
    ) -> bool:
        ...
        if (
            otp.created_at + timedelta(minutes=self.otp_expire_minutes)
            < datetime.utcnow()
        ):
            await self.increment_verification_attempts(user_id, purpose)
    ...
    async def invalidate_user_otps(
        self, user_id: int, purpose: str, db: AsyncSession
    ) -> None:
    ...
    async def get_otp_status(
        ...
        remaining_attempts = self.max_attempts - (
            int(current_attempts) if current_attempts else 0
        )
