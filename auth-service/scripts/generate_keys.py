...
3. Ensure proper file permissions (600 for private key)
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("auth-service/keys"),
        help="Output directory for key files (default: auth-service/keys)",
    )
    ...
    try:
        ...
        logger.info(f"   - JWT_PRIVATE_KEY_PATH={args.output_dir}/private_key.pem")
        logger.info(f"   - JWT_PUBLIC_KEY_PATH={args.output_dir}/public_key.pem")
        if os.name != "nt":
